create or replace PACKAGE PKG_REC_AFL_SIT IS
    ...
    FUNCTION F_GET_CLIENTES_QUERY(P_EMAIL IN VARCHAR2, P_NOM IN VARCHAR2, P_APAT IN VARCHAR2, P_AMAT IN VARCHAR2,
            P_IDSOCIO IN VARCHAR2, P_SORT_BY IN VARCHAR2, P_ORDER_BY IN VARCHAR2 )  RETURN VARCHAR2;

    FUNCTION F_GET_CLIENTES_COUNT(P_EMAIL IN VARCHAR2, P_NOM IN VARCHAR2,
            P_APAT IN VARCHAR2, P_AMAT IN VARCHAR2, P_IDSOCIO IN VARCHAR2 ) RETURN NUMBER;

    PROCEDURE P_GET_CLIENTES_VIEW( P_CURSOR OUT GLOBALPKG.RCT1, P_EMAIL IN VARCHAR2, P_NOM IN VARCHAR2,
            P_APAT IN VARCHAR2, P_AMAT IN VARCHAR2, P_IDSOCIO IN VARCHAR2, P_SORT_BY IN VARCHAR2,
            P_ORDER_BY IN VARCHAR2, P_LIMIT_N IN NUMBER, P_OFFSET_N IN NUMBER);
...
-----------------------------------------------------------------
    FUNCTION  F_GET_CLIENTES_QUERY(P_EMAIL IN VARCHAR2, P_NOM IN VARCHAR2, P_APAT IN VARCHAR2,
            P_AMAT IN VARCHAR2, P_IDSOCIO IN VARCHAR2, P_SORT_BY IN VARCHAR2, P_ORDER_BY IN VARCHAR2)
            RETURN VARCHAR2 IS
        FILTROS  VARCHAR2(10000) := ' ';
    BEGIN
        IF P_EMAIL IS NOT NULL THEN
            FILTROS := FILTROS||' AND EMAIL = '''||P_EMAIL||''' ';
        END IF;
        IF P_NOM IS NOT NULL THEN
            FILTROS := FILTROS||' AND SONOMBRE = '''||P_NOM||''' ';
        END IF;
        IF P_APAT IS NOT NULL THEN
            FILTROS := FILTROS||' AND APELLIDOPATERNO = '''||P_APAT||''' ';
        END IF;
        IF P_AMAT IS NOT NULL THEN
            FILTROS := FILTROS||' AND APELLIDOMATERNO = '''||P_AMAT||''' ';
        END IF;
        ...
        RETURN 'WITH AF AS (SELECT so.so_tipo_str ESTATUSSOCIO,SO_TEL4_STR TELEFONO,SO.TI_CVE_N CVETIENDA,SO_CALLE_STR CALLE,SO_NUM_STR NOEXTERIOR,SO_NUMINT_STR NOINTERIOR,
            ...
            SELECT * FROM  INFPED I where (ESTATUSPEDIDO = ''C'' OR PEDIDO IS NULL) and ESTATUSSOCIO != ''R'''||FILTROS;
    END;
-----------------------------------------------------------------
    FUNCTION F_GET_CLIENTES_COUNT(P_EMAIL IN VARCHAR2, P_NOM IN VARCHAR2,
            P_APAT IN VARCHAR2, P_AMAT IN VARCHAR2, P_IDSOCIO IN VARCHAR2) RETURN NUMBER IS
    ...
    BEGIN
        SQL_QUERY:=' SELECT COUNT(*) FROM ( '|| PKG_REC_AFL_SIT.F_GET_CLIENTES_QUERY(P_EMAIL, P_NOM,
            P_APAT, P_AMAT, P_IDSOCIO,NULL, NULL ) ||' ) ';
...
-----------------------------------------------------------------
    ...
    BEGIN
          QUERY_BASE := PKG_REC_AFL_SIT.F_GET_CLIENTES_QUERY(P_EMAIL, P_NOM,
            P_APAT, P_AMAT, P_IDSOCIO,P_SORT_BY, P_ORDER_BY );
