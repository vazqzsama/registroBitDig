<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
		...
		(function () {
			...
			this.ajaxRequest = function (params) {
				if (!isValido) {
					...
					system.service({
						url: cliente.ajaxUrl,
						data: { filtro: cliente.filtroRq, pager: params, total: cliente.total },
			...
			function getFiltro() {
				if ($("#idSocio").val().length == 0 && $("#correo").val().length == 0 && $("#soNombre").val().length == 0
					&& $("#aPaterno").val().length == 0 && $("#aMaterno").val().length == 0 && $("#statusP").val().length == 0) {
					isValido = false;
				} else {
					isValido = true;
					cliente.filtroRq = {
			...
			function clickConsultar() {
				...
				if (isValido) {
					...
					psDialog.error("Es necesario un parametro de busqueda");
			}
			...
			this.accion = function (value, row) {
				...
				if (row.soTipoStr == 'N' && (row.estatusPedido == 'C' || row.estatusPedido == null || row.estatusPedido == 'V')) {
	...
	<div id="divPanelPricipal" class="panel panel-blue">
		...
		<div id="tableContainer" style="padding-top: 0em;" class="panel-body padding-0">
			<table id="tablePendientes" data-toolbar-name="tablePendientesTB" data-show-columns="false"
				data-row-style="system.rowStyle" data-pagination="true" data-page-size="25"
				data-page-list="[25,50,75,100]" data-unique-id="idSocio" data-maintain-selected=true
				data-icons="system.icons" data-sticky-header="true" data-show-export="true" data-export-data-type="all">
				<thead>
					<tr>
						<th data-field="idSocio" data-sortable="true" data-switchable="false" data-align="center">Id
							Socio</th>

						<th data-field="falta" data-sortable="true" data-switchable="false"
							data-formatter="cliente.dateFormat" data-align="center">Fecha Registro</th>

						<th data-field="soTipoStr" data-align="center" data-formatter="cliente.estatusSocio"
							data-sortable="true">Estatus Socio</th>

						<th data-field="soNombre" data-align="center" data-formatter="cliente.fullName"
						...
						<th data-field="pedido" data-sortable="true" data-formatter="cliente.pedido"
							data-align="center">Pedido</th>

						<th data-field="estatusPedido" data-align="center" data-formatter="cliente.estatusPedido"
							data-sortable="false">Estatus Pedido</th>

						<th data-field="ref" data-align="center" data-sortable="false">Referencia</th>

						<th data-align="center" data-formatter="cliente.direccion" data-sortable="false">Direccion</th>

						<th data-align="center" data-formatter="cliente.accion" data-sortable="false">Acción</th>
